...
| **Kunden-Logins (Debitor) je Shop** | BC-App 1.0.22.0: Tabelle `Nop Customer` (Store + Customer No. + E-mail + Initial Password + Status Draft/Active), Seite `Nop Commerce Customers`, Aktion „Customers“ (Setup + Shops-Liste, Navigation); mehrere Logins pro Debitor möglich. Offen: Login-Transfer nach nop (Plugin-Endpoint) |
