...
| **Kunden-Logins (Debitor) je Shop** | BC-App 1.0.23.0: Tabelle `Nop Customer` — PK `(Shop Code, Customer No., E-mail)` ⇒ mehrere Logins je Debitor (eine Zeile je E-mail/Login); Seite `Nop Commerce Customers`, Aktion „Customers“ (Setup + Shops-Liste). Offen: Login-Transfer nach nop (Plugin-Endpoint) |
