...
| **Login-Transfer (Kunden-Logins)** | BC-App 1.0.30.0 + Plugin `POST /api/bc/customers/register`: BC legt nop-Kundenkonten mit E-mail + Initial-Passwort + Name an (mehrere Logins je Debitor: PK Shop/Kundennr./E-mail); `Push Logins` (Draft→Active, Nop Customer Id). 1.0.29.0: Kunden-Seite zeigt „Initial Password“ + „Last Sync Error“, Push-Meldung unterscheidet „keine Logins angelegt“ vs. „alle bereits aktiv“. 1.0.30.0: Passwort maskiert angezeigt + Eingabe über Dialog-Seite 62123; Aktion „Push Login“ einzeln (legt Konto an bzw. sync bei bereits existierender E-Mail die nopCommerce-Kunden-Id zurück nach BC). Auftrags-Import ordnet per Login-E-mail dem Bill-to-Kunden zu |
