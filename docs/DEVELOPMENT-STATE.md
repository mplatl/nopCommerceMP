...
| **Login-Transfer (Kunden-Logins)** | BC-App 1.0.29.0 + Plugin `POST /api/bc/customers/register`: BC legt nop-Kundenkonten mit E-mail + Initial-Passwort + Name an (mehrere Logins je Debitor: PK Shop/Kundennr./E-mail); `Push Logins` (Draft→Active, Nop Customer Id). 1.0.29.0: Kunden-Seite zeigt „Initial Password“ + „Last Sync Error“ (Passwort war bisher nicht eintragbar), Push-Meldung unterscheidet „keine Logins angelegt“ vs. „alle bereits aktiv“. Auftrags-Import ordnet per Login-E-mail dem Bill-to-Kunden zu |
