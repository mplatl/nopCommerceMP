| **Kategorien (Shop-eigene Hierarchie)** | BC-App 1.0.33.0: Shop-eigene Tabelle `Nop Category` (62130, PK Shop Code + Code) — Kategorien werden NICHT 1:1 mit BC-Item-Kategorien gematcht; wie in nopCommerce Parent-Kategorien (gleicher Shop, beliebige Tiefe). Validierung: Parent existiert im Shop, kein Selbst-Bezug/Zyklus, Loeschen/Umbenennen nur ohne Kinder. Seite `Nop Commerce Categories` (62132, Parent-Lookup gefiltert, Parent-Name-Spalte), Einstieg „Categories“ auf Shop-Karte + Shops-Liste. Push-Endpoint im Plugin folgt |
| **Kategorien-Push (BC → nop)** | BC-App 1.0.34.0 + Plugin `POST /api/bc/categories/register`: legt je Shop die Draft-Kategorien in nopCommerce an (Name + Beschreibung + nop-Parent-Id, `bcCategoryCode`-Attribut zur Rückverfolgung; existiert Name+Parent → `created:false`). BC pusht **Eltern vor Kindern** (Passes über die Draft-Zeilen), Status Draft→Active + `Nop Category Id`, „Push Categories“ auf Shop-Karte/Shops-Liste, „Push Category“ pro Zeile. Nächster Schritt: BC-Artikel → Shop-Kategorien zuweisen |
...
| **Login-Transfer (Kunden-Logins)** | BC-App 1.0.31.0 + Plugin `POST /api/bc/customers/register`: BC legt nop-Kundenkonten mit E-mail + Initial-Passwort + Name an (mehrere Logins je Debitor: PK Shop/Kundennr./E-mail); `Push Logins` (Draft→Active, Nop Customer Id). 1.0.30.0: Passwort maskiert (Dialog 62123), „Push Login“ einzeln, nop-Kunden-Id-Sync. 1.0.31.0: Plugin schreibt Namen in die sichtbaren nop-Spalten `Customer.FirstName/LastName` (statt totem Generic-Attribut) — LastName = BC-Kundennr. → Kunde in nop-Liste/Details identifizierbar; BC sendet bei leerem Namen die Kundennr. 1.0.32.0: Passwort-Eingabe wie Standard (Benutzer): Spalte maskiert (`ExtendedDatatype=Masked`), Eingabe per AssistEdit → StandardDialog 62123 (maskiertes Feld, System-OK via `ACTION::OK`/OnQueryClosePage, kein eigener OK-Button) |
...
| **Kunden-Logins (Debitor) je Shop** | BC-App 1.0.25.0: Tabelle `Nop Customer Login` (62116), Seite `Nop Commerce Customers` — Einstieg über Store („Customers“) **und** vorbereitet über Debitor (Filter `SetCustomerNo`, Shop-Code-Spalte sichtbar); PageExtensions auf `Customer Card/List` im Repo geplant, lokal nicht kompilierbar (Symbol-Stubs ohne Base-Objekte). Offen: Login-Transfer nach nop (Plugin-Endpoint) |
